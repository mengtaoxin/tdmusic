...
test('playlist list navigates to playlist detail by name', async ({ page }) => {
  ...
  await expect(page.getByText('Sample 1')).toHaveCount(0)
...
test('artist list navigates to albums, album tracks, and all music', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Artist 1' })).toBeVisible()
  await expect(page.getByText('Sample 1')).toHaveCount(0)

  await page.getByRole('link', { name: 'Artist 1' }).click()
  await expect(page).toHaveURL(/\/artists\/Artist(%20|\+)1\/albums/)
  await expect(page.getByRole('heading', { name: 'Artist 1' })).toBeVisible()
  ...
  await expect(page).toHaveURL(/\/artists\/Artist(%20|\+)1\/albums\/Album(%20|\+)1/)
  await expect(page.getByRole('heading', { name: 'Album 1' })).toBeVisible()
  await expect(page.getByText('Sample 1')).toBeVisible()

  await page.goto('/artists/Artist%201/albums')
  await page.getByRole('link', { name: 'All music by this artist' }).click()
  await expect(page).toHaveURL(/\/artists\/Artist(%20|\+)1$/)
  await expect(page.getByRole('heading', { name: 'Artist 1' })).toBeVisible()
  await expect(page.getByText('Sample 1')).toBeVisible()
})