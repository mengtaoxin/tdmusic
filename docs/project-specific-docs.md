...
- `playlists` is an array of playlists; each references tracks by `id` (resolved against accepted tracks only).
...
- `/artists` lists artists (virtualized rows via `v-virtual-scroll` + `useVirtualListHost`, same host-height pattern as Music List); `/artists/{name}/albums` lists that artist’s albums via shared `AlbumGallery` (same virtualized cover gallery as `/albums`) plus **All music by this artist** above the gallery (escape hatch when album metadata is wrong); `/artists/{name}/albums/{album}` shows that album’s tracks; `/artists/{name}` shows all tracks by the artist.
...
- Single `<audio>` in `AudioHost` (mounted from `App.vue`) so playback survives route changes. Queue/shuffle/persist live in the player store; load/resolve/enrich/prefetch/failure-skip live in `lib/playback/playbackSession`; binding, media events, and Media Session hooks live in `lib/playback/playbackTransport` (`createPlaybackTransport`). `AudioHost` only wires the element, Pinia watchers, and transport.
...
- If another app / the OS pauses the element, transport clears `pendingPlay` and `playing` so the next play click re-invokes `audio.play()` (otherwise `pendingPlay` stayed true and the watcher did not re-run).
